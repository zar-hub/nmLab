...
function plotPeaksC1S_compAndCO(coeff, res, [multiComp, app])
	// multiComp	= 1 plot each component separately
	//				= 0 plot peak used in CO estimation
	// app = 1 append the waves to the current graph
	
	wave coeff, res
	int multiComp, app
	multiComp = paramisDefault(multiComp) ? 1 : multiComp
	
	wave CO 	= $getGlobalWave("CO", like = res)
	wave C1S 	= $getGlobalWave("C1S", like = res)
	wave C1 	= $getGlobalWave("C1", like = res)
	wave C2 	= $getGlobalWave("C2", like = res)
	wave C3	= $getGlobalWave("C3", like = res)

	duplicate/free coeff tcoeff
	make/free toZeroList = {0,1,5,10,15,20,25}
	...
	if (multiComp == 1) // plot components
		tcoeff = coeff
		for (int i : toZeroList)
			tcoeff[i] = 0
		endfor
